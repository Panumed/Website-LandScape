    ...
    <div v-if="step" class="w-full">
      ...
      <div v-if="step.type === 'warning'" class="flex flex-col items-center space-y-8">
        <div v-if="step.images" class="flex flex-row justify-center items-center gap-4 w-full">
          <img v-for="img in step.images" :key="img" :src="getImageUrl(img)" class="h-32 object-contain" alt="" />
        </div>
        <p class="text-gray-700 text-xl leading-relaxed whitespace-pre-line text-center">{{ parsedText }}</p>
        <button @click.stop="handleStart" class="text-green-600 hover:text-green-800 transition-colors mt-8 font-medium">
      ...
      <div v-else-if="step.type === 'onboarding'" class="flex flex-col w-full px-2 py-4">
        ...
        <div class="w-full space-y-6 text-left">
          <div class="space-y-2">
            ...
            <input v-model="formData.name" type="text" placeholder="คำตอบ" class="w-full px-6 py-2 rounded-full border-[1.5px] border-gray-400 focus:outline-none focus:border-gray-500 bg-transparent text-lg placeholder-gray-400 text-gray-800" />
          </div>
          <div class="space-y-2">
            <label class="block text-gray-700 text-xl pl-2">อายุ</label>
            <input v-model="formData.age" type="number" placeholder="คำตอบ" class="w-full px-6 py-2 rounded-full border-[1.5px] border-gray-400 focus:outline-none focus:border-gray-500 bg-transparent text-lg placeholder-gray-400 text-gray-800" />
          </div>
          <div class="space-y-2">
            <label class="block text-gray-700 text-xl pl-2">เพศ</label>
            <div class="relative w-36">
              <select v-model="formData.gender" class="w-full pl-6 pr-10 py-2 rounded-full border-[1.5px] border-gray-400 focus:outline-none focus:border-gray-500 bg-transparent appearance-none text-lg text-gray-800">
        ...
        <div class="w-full flex justify-end mt-16">
          <button v-if="formData.name" @click.stop="saveOnboarding" class="px-8 py-2 border-[1.5px] border-gray-400 rounded-full text-gray-800 hover:bg-gray-100 transition-colors text-xl">
            ตกลง
          </button>
        </div>
      </div>
      ...
      <div v-else-if="step.type === 'menu'" class="flex flex-col items-center space-y-8">
        ...
        <div class="flex flex-col space-y-4 w-full">
          <button 
            ...
            class="w-full px-4 py-3 rounded-full border border-gray-300 hover:bg-gray-100 transition-colors text-center text-gray-600 text-lg"
          >
            {{ opt.text }}
          </button>
        </div>
      ...
      <div v-else-if="step.type === 'text'" class="flex flex-col items-center space-y-8 w-full">
        ...
        <input 
          ...
          class="w-full px-6 py-3 rounded-full border border-gray-300 focus:outline-none focus:ring-2 focus:ring-blue-200 text-center bg-transparent text-lg placeholder-gray-400"
